...
BEGIN
    IF to_regclass('public.media_strategy') IS NOT NULL THEN
        -- Demo mock row patch: add a complete snapshot_watermark node to the default strategy when it was loaded before this plugin existed.
        -- 演示数据补丁：为此前已加载的默认策略补齐完整截图水印节点。
...
snapshot_watermark:
  ...
  opacity: 0.75',
        ...
        WHERE "name" = '默认直播录制策略'
          ...
          AND ("strategy" NOT LIKE '%snapshot_watermark:%' OR "strategy" NOT LIKE '%fontSize:%');
...
snapshot_watermark:
  ...
  opacity: 0.65',
        ...
        -- Demo mock row patch: expand the park-security strategy from the old minimal snapshot_watermark node.
        -- 演示数据补丁：将园区安防策略从旧版简略截图水印节点补齐为完整配置。
        UPDATE media_strategy
        SET
            "enable" = 1,
            "strategy" = 'record:
