...
正常插件安装会加载`manifest/sql/003-cms-starter-content.sql`中的 starter 内容。
因此新安装的 CMS 会直接带有参考站点配置、栏目树、轮播图、友情链接、饱满文章正文
以及 3 条默认可见的审核通过留言。`manifest/sql/mock-data/`仍作为本地校验场景的
可选重置数据。
...
`manifest/sql/003-cms-starter-content.sql`内置的 starter 示例站点。该能力适用于
