// This file keeps linapro-content-notice API DTO boundary checks inside the plugin module.
...
import (
	...
	noticev1 "lina-plugin-linapro-content-notice/backend/api/notice/v1"
)

// TestContentNoticeAPIsDoNotDependOnGeneratedEntities ensures public API contracts do not import database entities.
func TestContentNoticeAPIsDoNotDependOnGeneratedEntities(t *testing.T) {
	assertNoGeneratedEntityImports(t, contentNoticePluginRoot(t))
}

// TestContentNoticeAPIDTOsDoNotUseEntityNaming ensures response DTOs use API-oriented names.
func TestContentNoticeAPIDTOsDoNotUseEntityNaming(t *testing.T) {
	assertNoEntityTypeNames(t, contentNoticePluginRoot(t))
}

// TestContentNoticeAPIDTOFilePlacement keeps shared DTOs in the API package main source file.
func TestContentNoticeAPIDTOFilePlacement(t *testing.T) {
	assertTypeDeclaredInFile(t, contentNoticePluginRoot(t), map[string]string{
		"notice/v1/NoticeItem": "notice.go",
	})
}

// TestContentNoticeAPIDTOFilesAvoidLegacyNames rejects legacy DTO-only file naming.
func TestContentNoticeAPIDTOFilesAvoidLegacyNames(t *testing.T) {
	assertNoLegacyDTOFiles(t, contentNoticePluginRoot(t))
}

// TestContentNoticeResponseDTOsHideInternalFields verifies implementation fields stay out of responses.
func TestContentNoticeResponseDTOsHideInternalFields(t *testing.T) {
	assertJSONFieldsAbsent(t, "notice.NoticeItem", reflect.TypeOf(noticev1.NoticeItem{}), internalResponseFields())
}

// TestContentNoticeAPIDocI18NDoesNotReferenceRemovedDTOFields keeps apidoc translations aligned with DTOs.
func TestContentNoticeAPIDocI18NDoesNotReferenceRemovedDTOFields(t *testing.T) {
	assertAPIDocI18NExcludesTokens(t, contentNoticePluginRoot(t), []string{
		"NoticeEntity",
		"deletedAt",
	})
}

// contentNoticePluginRoot returns the plugin root directory for path-based contract checks.
func contentNoticePluginRoot(t *testing.T) string {
	t.Helper()
