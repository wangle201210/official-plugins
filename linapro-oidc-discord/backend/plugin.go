// Package backend wires the linapro-oidc-discord source plugin into the host
...
const portalGroupPath = "/portal/linapro-oidc-discord"

// init registers the linapro-oidc-discord source plugin, declares its
// external-identity provider ownership, and registers HTTP routes.
func init() {
	plugin := pluginhost.NewDeclarations(pluginID)
	plugin.Assets().UseEmbeddedFiles(pluginoidcdiscord.EmbeddedFiles)
...
// OIDC routes on the portal route group (login-start redirects to Discord,
// the callback exchanges the code with the host external-login seam), plus
// the PROTECTED admin settings API under the plugin API prefix guarded by
...
func registerRoutes(ctx context.Context, registrar pluginhost.HTTPRegistrar) error {
	...
	if services == nil {
		return gerror.New("linapro-oidc-discord routes require host external-login capability")
	}
	authSvc := services.Auth()
	if authSvc == nil || authSvc.ExternalLogin() == nil {
		return gerror.New("linapro-oidc-discord routes require host external-login sub capability")
	}
	hostConfigSvc := services.HostConfig()
	if hostConfigSvc == nil || hostConfigSvc.SysConfig() == nil {
		return gerror.New("linapro-oidc-discord routes require host sys_config capability")
	}
	logger.Infof(ctx, "linapro-oidc-discord registering portal routes group=%s", portalGroupPath)
