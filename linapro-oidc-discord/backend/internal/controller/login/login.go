// Package login implements the HTTP entry points for the Discord OIDC login
// flow exposed by the linapro-oidc-discord plugin. The controller is
...
// ControllerV1 is the linapro-oidc-discord login controller. It handles the
// browser-facing login-start and callback routes registered on the portal
// route group.
type ControllerV1 struct {
	oauthSvc    oauthsvc.Service    // oauthSvc orchestrates the Discord OIDC login flow.
	settingsSvc settingssvc.Service // settingsSvc supplies SSO delivery rules and the SPA landing path at request time.
}

// NewV1 creates and returns one Discord OIDC login controller instance.
