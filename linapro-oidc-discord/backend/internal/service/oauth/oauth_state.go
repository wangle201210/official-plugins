...
// by the Discord OIDC login flow. The state token embeds the optional business
...
// through Discord, which removes an entire class of SameSite/proxy cookie
// failures.
...
const stateMACKeyPrefix = "linapro-oidc-discord::"
...
func (c *hmacStateCodec) Encode(_ context.Context, stateKey string, returnTo string, clientSecret string) (string, error) {
	...
	if _, err := rand.Read(nonceBuffer); err != nil {
		return "", bizerr.WrapCode(gerror.Wrap(err, "oidc discord: state entropy read failed"), CodeStateGenerateFailed)
	}
	...
	if err != nil {
		return "", bizerr.WrapCode(gerror.Wrap(err, "oidc discord: encode state payload failed"), CodeStateGenerateFailed)
	}
...
func (c *hmacStateCodec) Decode(_ context.Context, state string, clientSecret string) (StatePayload, error) {
	...
	if len(parts) != 2 {
		return StatePayload{}, bizerr.WrapCode(gerror.New("oidc discord: state token is malformed"), CodeCallbackStateMismatch)
	}
	expected := computeStateMAC(parts[0], clientSecret)
	if !hmac.Equal([]byte(parts[1]), []byte(expected)) {
		return StatePayload{}, bizerr.WrapCode(gerror.New("oidc discord: state signature mismatch"), CodeCallbackStateMismatch)
	}
	raw, err := base64.RawURLEncoding.DecodeString(parts[0])
	if err != nil {
		return StatePayload{}, bizerr.WrapCode(gerror.Wrap(err, "oidc discord: decode state payload failed"), CodeCallbackStateMismatch)
	}
	var payload StatePayload
	if err = json.Unmarshal(raw, &payload); err != nil {
		return StatePayload{}, bizerr.WrapCode(gerror.Wrap(err, "oidc discord: parse state payload failed"), CodeCallbackStateMismatch)
	}
	if payload.ExpiresAt > 0 && time.Now().Unix() > payload.ExpiresAt {
		return StatePayload{}, bizerr.WrapCode(gerror.New("oidc discord: state has expired"), CodeCallbackStateMismatch)
	}
	return payload, nil
}
