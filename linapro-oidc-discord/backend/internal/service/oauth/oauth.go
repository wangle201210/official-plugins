...
type CallbackOutput struct {
	// Handoff is the single-use host code the SPA exchanges for tokens.
	Handoff string
	// AccessToken is retained only for optional admin-configured SSO receivers.
	AccessToken string
	...
	PreToken string
	// TenantCandidates lists selectable tenants (also embedded in handoff).
