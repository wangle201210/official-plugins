...
- 后端 controller 方法会被自动发现为 `Before*` 前置处理器和 `After*` 通知处理器（命名规则与源码插件一致），并通过运行时日志展示生命周期流程
...
`manifest`宿主服务示例授权`config/`打包清单前缀。`/api/v1/manifest-demo`路由会通过`manifest.get`读取`config/profile.yaml`和`config/config.yaml`，并在内嵌页面展示返回的 profile 与配置预览，从而完整演示从声明到使用的流程。`/api/v1/host-call-demo` 路由在同一前缀下额外演示 `manifest.get_many`、`manifest.list`，以及 `bizctx`、`cache`、`lock` 宿主服务的低风险调用。

运行期插件配置通过 `Plugins().Config()` 读取，授权标识为 `plugins.config.get`。SQL 和 i18n 资源不纳入本次示例。

该样例还通过 `service: jobs` 和 `method: jobs.register` 声明一个内置定时任务。`RegisterPlugin` 函数通过 `plugin.Jobs().Register(...)` 完成声明；宿主 Jobs 发现阶段使用 `pluginbridge.NewDeclarations()` 执行该声明，将结果投影到 Jobs 管理中，并通过声明的 `JobHeartbeat` 路由执行心跳任务。

同一个 `jobs` 宿主服务声明还授权后端测试使用运行期 `pluginbridge.Default().Jobs()` 领域方法，包括批量读取、有界列表、可见性校验、创建、更新、删除、运行和状态变更等操作。
