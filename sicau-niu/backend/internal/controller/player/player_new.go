...
// controller holds the identity, college and activation services as fields
// injected at route assembly time; it never constructs services on the request
// path. Player endpoints read the authenticated player ID from the request
// context populated by the plugin player-auth middleware, ensuring each player
// only reads and writes their own data.
