// activation_guard.go implements the transactional anti-cheat guards for photo
// check-ins: the daily attempt quota (counting failed attempts) that
...
// guards run under the player's row lock, consume one operator-rule snapshot
// shared with LBS matching, and never expose distance or bearing hints.
