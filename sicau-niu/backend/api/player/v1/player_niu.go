...
// visible-cattle map list: the currently visible cattle (filtered by release
// schedule) with each cattle's GPS anchor, shared-pool activation state and
...
type VisibleNiuReq struct {
	g.Meta `path:"/plugins/sicau-niu/player/niu" method:"get" tags:"Sicau Niu Player" summary:"List visible cattle for the map" dc:"Return the cattle currently visible to the authenticated player, filtered by release schedule (release stage set, online time reached, optional weekday/time window matched). Each item carries its GPS anchor, shared-pool activation status and whether the current player has already activated it. The visible set is bounded (≤120) and returned in one response; statuses are batch-assembled to avoid N+1. Requires a valid player token."`
}
