...
type DeleteNiuReq struct {
	g.Meta `path:"/plugins/sicau-niu/admin/niu/{id}" method:"delete" tags:"Sicau Niu Admin" summary:"删除牛只" dc:"Soft-delete a cattle and cascade soft-delete its unique main card in one transaction, so no dangling card remains. Both are recoverable. Protected by host unified permission check." permission:"sicau-niu:niu:delete"`
