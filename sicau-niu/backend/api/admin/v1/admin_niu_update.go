...
type UpdateNiuReq struct {
	g.Meta          `path:"/plugins/sicau-niu/admin/niu/{id}" method:"put" tags:"Sicau Niu Admin" summary:"修改牛只" dc:"Update a cattle's attributes and online-time visibility rules. The code must stay unique among active cattle; type/subtype/college rules match creation. Status is not changed here. Protected by host unified permission check." permission:"sicau-niu:niu:update"`
