...
| 实体 | 关键字段 | 说明 |
|------|---------|------|
| `Niu` 牛 | 序号、类型（普通/特殊）、特殊子类（学院/贡献/校友/精神）、名称、关联院系、GPS 经纬、上线时间、可见窗口（周几/时段）、关联卡片、状态、首发用户、激活序号 | 共享池；状态全员可见；锚点固定 |
...
flowchart TD
  ...
  C2["C2 niu-catalog-admin<br/>牛/铁牛/卡片/金句模型+后台配置+上线时间(M4数据,M5)"]
...
| `niu-catalog-admin` | M4(数据), M5 | 牛/铁牛/卡片/金句表与后台 CRUD、上线时间和可见窗口配置、铁牛标识登记、看板雏形 |
