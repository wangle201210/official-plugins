...
type RuntimeTokenIssueReq struct {
	g.Meta   `path:"/api/v1/token/get" method:"post" tags:"UIdentity Token Runtime" summary:"Issue runtime access token" dc:"Validate application secret, account password and access rules, then issue a short-lived runtime access token for non-page clients."`
...
type RuntimeTokenInfoReq struct {
	g.Meta      `path:"/api/v1/token/getUserInfoByToken" method:"get" tags:"UIdentity Token Runtime" summary:"Get user info by runtime token" dc:"Validate a runtime access token and return the primary and delegated account projections available for the token application."`
