...
type ActivationStartReq struct {
	g.Meta `path:"/api/v1/activate/baseInfo" method:"post" tags:"UIdentity Activation" summary:"Start account activation" dc:"Verify account number, display name and identity card, then create a short-lived activation challenge in plugin token storage."`
...
type ActivationFaceReq struct {
	g.Meta      `path:"/api/v1/activate/face" method:"post" tags:"UIdentity Activation" summary:"Record activation face proof" dc:"Record a local face proof marker or image reference for an activation challenge without requiring an external face service by default."`
...
type ActivationPhoneReq struct {
	g.Meta      `path:"/api/v1/activate/phone" method:"post" tags:"UIdentity Activation" summary:"Bind activation phone" dc:"Verify an activation SMS code, bind a phone number to the account attached to the challenge, and mark the account normal."`
	ChallengeId string `json:"uuid" v:"required" dc:"Activation challenge ID" eg:"act_abcdef"`
...
type ActivationWechatStateCreateReq struct {
	g.Meta      `path:"/api/v1/activate/wechatQr" method:"post" tags:"UIdentity Activation" summary:"Create activation Wechat state" dc:"Create a Wechat activation binding state for one activation challenge and return the configured external authorization URL when available."`
	ChallengeId string `json:"uuid" v:"required" dc:"Activation challenge ID" eg:"act_abcdef"`
...
type ActivationStateReq struct {
	g.Meta      `path:"/api/v1/activate/state" method:"post" tags:"UIdentity Activation" summary:"Get activation state" dc:"Read the current activation challenge state and account status."`
	ChallengeId string `json:"uuid" v:"required" dc:"Activation challenge ID" eg:"act_abcdef"`
}

// ActivationStartRes returns activation challenge metadata.
type ActivationStartRes struct {
	UUID string `json:"uuid" dc:"Activation challenge ID" eg:"act_abcdef"`
	Face bool   `json:"face" dc:"Whether this account should collect face proof before completion" eg:"true"`
}
...
// ActivationFaceStepRes returns face activation verification state.
type ActivationFaceStepRes struct {
	UUID string `json:"uuid" dc:"Activation challenge ID" eg:"act_abcdef"`
	Pass bool   `json:"pass" dc:"Whether the face step passed" eg:"true"`
	Msg  string `json:"msg" dc:"Face verification message" eg:""`
}

// ActivationFaceRes returns face activation step state.
type ActivationFaceRes = ActivationFaceStepRes
...
type ActivationWechatStateCreateRes struct {
	UUID   string `json:"uuid" dc:"Activation Wechat state, equal to the activation challenge ID" eg:"act_abcdef"`
	QRCode string `json:"qrcode" dc:"External authorization URL when runtime.activationWechatAuthorizeUrl is configured" eg:"https://wechat.example.com/oauth?state=act_abcdef"`
}
