...
type AccountPasswordUnlockReq struct {
	g.Meta         `path:"/api/v1/account/unlockPassword" method:"post" tags:"UIdentity CAS" summary:"Unlock account password failures" dc:"Clear short-lived password failure counters for visible account numbers, matching the legacy account unlockPassword backend behavior." permission:"uidentity:cas:write"`
...
type AccountPasswordChallengeReq struct {
	g.Meta `path:"/api/v1/account/updatePasswordGetUser" method:"post" tags:"UIdentity CAS" summary:"Create password reset challenge" dc:"Create a short-lived password reset challenge for the account number. The challenge is stored in plugin-owned OAuth token storage and must be verified by phone before password reset."`
...
type AccountPasswordChallengeRes struct {
	UUID   string `json:"uuid" dc:"Password reset challenge ID" eg:"b1803e7b5a9f4d3fb3c60ee09f18f045"`
...
type AccountPasswordPhoneVerifyRes struct {
	UUID string `json:"uuid" dc:"Verified password reset challenge ID" eg:"b1803e7b5a9f4d3fb3c60ee09f18f045"`
}

// AccountPasswordSelfResetReq defines the final self-service password reset request.
type AccountPasswordSelfResetReq struct {
	g.Meta      `path:"/api/v1/account/updatePasswordBySelf" method:"post" tags:"UIdentity CAS" summary:"Reset password by verified challenge" dc:"Reset the account password after a password reset challenge has been verified. The challenge is consumed after successful reset."`
	ChallengeId string `json:"uuid" v:"required" dc:"Verified password reset challenge ID" eg:"b1803e7b5a9f4d3fb3c60ee09f18f045"`
