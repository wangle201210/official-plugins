...
type LegacyLogSnapshotReq struct {
	g.Meta `path:"/api/v1/log/watch" method:"get" tags:"UIdentity Legacy Operations" summary:"Get legacy log snapshot" dc:"Read a bounded tail snapshot from a configured plugin log directory instead of opening an unbounded SSE file watcher." permission:"uidentity:cas:read"`
...
type LegacyUploadFile struct {
	...
	Path     string `json:"path" dc:"Plugin-local public path" eg:"/uploads/2026/06/file.png"`
	FullPath string `json:"full_path" dc:"Full public path. The default local implementation returns the same value as path." eg:"/uploads/2026/06/file.png"`
