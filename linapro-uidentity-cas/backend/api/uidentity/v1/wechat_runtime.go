...
type WechatLoginQRReq struct {
	g.Meta   `path:"/api/v1/cas/getLoginQr" method:"post" tags:"UIdentity Wechat Runtime" summary:"Create Wechat QR login state" dc:"Create a short-lived Wechat QR login state and return the configured external authorization URL when available."`
	ClientId string `json:"appid" v:"required" dc:"Application client ID" eg:"portal"`
	Callback string `json:"CasCallback" dc:"Optional legacy cascallback value echoed to the configured redirect URL" eg:"choose"`
}

// WechatLoginCallbackReq defines QR login callback completion.
type WechatLoginCallbackReq struct {
	g.Meta   `path:"/api/v1/wechat/loginCallback" method:"get" tags:"UIdentity Wechat Runtime" summary:"Complete Wechat QR login callback" dc:"Record a Wechat QR callback result. If unionId is supplied, the plugin completes login or creates a bind challenge; otherwise it records a structured unsupported-flow result."`
	State    string `json:"state" v:"required" dc:"Wechat QR login state" eg:"loginByQr_abcdef"`
	ClientId string `json:"appid" dc:"Optional application client ID used to guard callback state" eg:"portal"`
...
type WechatLoginQRResultReq struct {
	g.Meta `path:"/api/v1/cas/getCasLoginQrRes" method:"post" tags:"UIdentity Wechat Runtime" summary:"Get Wechat QR login result" dc:"Read the current Wechat QR login state. Terminal states are consumed after this read."`
	State  string `json:"state" v:"required" dc:"Wechat QR login state" eg:"loginByQr_abcdef"`
}
...
type WechatLoginQRResultRes struct {
	...
	ChallengeId string              `json:"uuid" dc:"Union ID bind challenge ID when status is bind_required" eg:"uid_abcdef"`
	CallbackUrl string              `json:"call_back_url" dc:"Union ID bind callback URL when a bind challenge was created" eg:"https://example.com/bind?uuid=uid_abcdef"`
