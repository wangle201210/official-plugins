...
func registerLegacyPublicRoutes(group pluginhost.RouteGroup, legacyController *uidentitycontroller.LegacyController) {
	...
	// The old registerApiRouter guarded the third-party user self-service
	// routes with APIMiddleWare (appid/ts/sign md5 signature). Keep that
	// boundary so client-supplied identity headers stay signature-bound.
	group.Group("/", func(signed pluginhost.RouteGroup) {
		signed.Middleware(legacyController.LegacyAPISignGuard)
		signed.POST("/user/changePassword", legacyController.UserChangePassword)
		signed.POST("/user/changePhone", legacyController.UserChangePhone)
		signed.POST("/user/changeEmail", legacyController.UserChangeEmail)
		signed.POST("/user/changeQQ", legacyController.UserChangeQQ)
		signed.POST("/user/unbindWechat", legacyController.UserUnbindWechat)
		signed.POST("/user/getUserInfo", legacyController.UserInfo)
		signed.GET("/user/getUserCasLoginLog", legacyController.UserLoginLogs)
		signed.GET("/user/accountAppList", legacyController.UserApplications)
		signed.GET("/user/accountAppRole", legacyController.UserAppRoles)
		signed.POST("/user/accountAppRole", legacyController.UserAppRoleCreate)
		signed.POST("/user/accountAppRoleUpdate", legacyController.UserAppRoleUpdate)
		signed.POST("/user/changeWechatQr", legacyController.UserChangeWechatQR)
		signed.POST("/user/changeWechatState", legacyController.UserChangeWechatState)
	})
