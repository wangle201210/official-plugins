// =================================================================================
// This file is auto-generated by the GoFrame CLI tool. You may modify it as needed.
// =================================================================================

package dao

import (
	"lina-plugin-linapro-uidentity-cas/backend/internal/dao/internal"
)

// sysOperaLogDao is the data access object for the table plugin_linapro_uidentity_cas_sys_opera_log.
// You can define custom methods on it to extend its functionality as needed.
type sysOperaLogDao struct {
	*internal.SysOperaLogDao
}

var (
	// SysOperaLog is a globally accessible object for table plugin_linapro_uidentity_cas_sys_opera_log operations.
	SysOperaLog = sysOperaLogDao{internal.NewSysOperaLogDao()}
)

// Add your custom methods and functionality below.
