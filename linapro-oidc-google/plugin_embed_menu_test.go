// This file verifies that Google OIDC menu i18n resources are embedded so the
...
func TestEmbeddedMenuJSONLocalizesSettingsTitle(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.path, func(t *testing.T) {
			...
			settings := payload.Menu["plugin:linapro-oidc-google:settings"]
			if settings.Title != tc.title {
				t.Fatalf("settings title: got %q want %q", settings.Title, tc.title)
			}
			if payload.Menu["plugin:linapro-oidc-google:settings-update"].Title == "" {
