// oauth_callback.go implements the Google callback step of the OIDC login
...
// cross-site round trip through Google.
...
func (s *serviceImpl) CompleteCallback(ctx context.Context, in CallbackInput) (*CallbackOutput, error) {
	...
	if code == "" {
		return nil, bizerr.WrapCode(gerror.New("oidc google: callback code is empty"), CodeCallbackCodeRequired)
	}
	if state == "" {
		return nil, bizerr.WrapCode(gerror.New("oidc google: callback state is empty"), CodeCallbackStateMismatch)
	}
	if s.stateCodec == nil {
		return nil, bizerr.WrapCode(gerror.New("oidc google: state codec is missing"), CodeCallbackStateMismatch)
	}
	...
	if identity == nil || strings.TrimSpace(identity.Subject) == "" {
		return nil, bizerr.WrapCode(
			gerror.New("oidc google: verified identity is missing subject"),
			CodeIdentityVerifyFailed,
		)
	}
	logger.Infof(
		ctx,
		"linapro-oidc-google callback verified provider=%s subject=%s email=%s",
		Provider,
		identity.Subject,
		identity.Email,
	)
