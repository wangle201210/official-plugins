...
test.describe("TC-1 media plugin owned E2E discovery", () => {
  ...
  test("TC-1b: 媒体策略绑定优先级和流别名接口可用", async () => {
  ...
  test("TC-1c: 媒体管理全部 REST 接口语义正确", async () => {
    const api = await createAdminApiContext();
    const suffix = Date.now().toString();
  ...
  test("TC-1d: 宿主静态入口可加载媒体管理页面", async ({ browser }) => {
