...
type SetRouteDataReq struct {
	g.Meta      `path:"/route-memories/{deviceCode}/{channelCode}" method:"put" tags:"路由记忆" summary:"写入路由数据" dc:"兼容 HotGo 路由记忆接口，按设备编号和通道编号保存路由数据。" access:"public"`
...
type GetRouteDataReq struct {
	g.Meta      `path:"/route-memories/{deviceCode}/{channelCode}" method:"get" tags:"路由记忆" summary:"读取路由数据" dc:"兼容 HotGo 路由记忆接口，按设备编号和通道编号读取路由数据。" access:"public"`
	InnerApiKey string `json:"X-Inner-Api-Key" in:"header" dc:"内部接口API Key；默认值media；显式配置innerapi.apiKey为空时可不传" eg:"media"`
	DeviceCode  string `json:"deviceCode" v:"required#设备编号不能为空" dc:"设备编号" eg:"34020000001320000001"`
	ChannelCode string `json:"channelCode" v:"required#通道编号不能为空" dc:"通道编号" eg:"34020000001320000001"`
}
...
type DelRouteDataReq struct {
	g.Meta      `path:"/route-memories/{deviceCode}/{channelCode}" method:"delete" tags:"路由记忆" summary:"删除路由数据" dc:"兼容 HotGo 路由记忆接口，按设备编号和通道编号删除路由数据。" access:"public"`
	InnerApiKey string `json:"X-Inner-Api-Key" in:"header" dc:"内部接口API Key；默认值media；显式配置innerapi.apiKey为空时可不传" eg:"media"`
	DeviceCode  string `json:"deviceCode" v:"required#设备编号不能为空" dc:"设备编号" eg:"34020000001320000001"`
	ChannelCode string `json:"channelCode" v:"required#通道编号不能为空" dc:"通道编号" eg:"34020000001320000001"`
}
