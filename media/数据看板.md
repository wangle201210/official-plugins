...
| 流信息列表 | `stream_list` | 按`source_type`、映射后的`node_id`或`instance_id`、`tenant_id`等条件查询`media_report_stream`得到 |
...
- 流信息列表：按`source_type`将请求`source_id`映射为`node_id`或`instance_id`，再结合`tenant_id`或`status`查询`media_report_stream`；`protocol_summary[].total_sessions`直接读取`protocol_summary`，当前活跃会话数按`media_report_session.close_time IS NULL`聚合覆盖。
- 会话列表：按`stream_id`、`tenant_id`、`protocol_type`、`node_id`或`instance_id`查询`close_time`为空的`media_report_session`，链路详情直接读取`link_hops`，链路总延迟优先按`link_hops[].latency_ms`求和。
