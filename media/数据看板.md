...
//总节点概览（节点列表）
{
  ...
  "data": {   
      ...
      "memory_allocated": 256,        // 当前节点内存分配量（单位：MB）
      "memory_used": 182.3,           // 当前节点内存使用量（单位：MB）
      "disk_io_read": 120,                 // 当前节点磁盘读 KB/S
      "disk_io_write": 80,                // 当前节点磁盘写 KB/S
      "network_in": 1054.6,                 // 当前节点入流量 KB/S
      "network_out": 3025.5,                // 当前节点出流量 KB/S
    ...
    "instance_list": [                     // 实例列表（每个实例对应一个进程/容器）
      {
        ...
        "memory_allocated": 8,             // 实例内存分配量（单位：MB）
        "memory_used": 5.2,                // 实例内存使用量（单位：MB）
        "disk_io_read": 120,                 // 实例磁盘读 KB/S
        "disk_io_write": 80,                // 实例磁盘写 KB/S
        "network_in": 1054.6,                 // 实例入流量 KB/S
        "network_out": 3025.5,                // 实例出流量 KB/S
    ...
    "stream_list": [                       // 流列表
      {
        ...
        "device_id": "34020000001320000001", // 设备国标ID
        ...
        "active_session_list": [
            {
              ...
              "device_id": "34020000001320000001",   // 会话所属设备国标ID
              "client_id": "user-88721",             // 客户端身份（设备ID）
              "client_ip": "1.202.33.41",
              "client_type": 1,                      // 1-mobile / 2-pc / 0-未知
...
| 会话列表 | `session_id`、`client_id`、`tenant_id`、`protocol_type`、播放质量字段 | `media_report_session`；`play_duration`由`start_time`到`close_time`或当前时间动态计算 |
...
| 流信息列表 | `source_type`、`source_id` | 请求条件回显；`source_id`按`source_type`映射为`media_report_stream.node_id`或`media_report_stream.instance_id` |
| 流信息列表 | `stream_list` | 按`source_type`、映射后的`node_id`或`instance_id`、`tenant_id`、`device_id`等条件查询`media_report_stream`得到 |
| 流信息列表 | `source_url`、`stream_id`、`stream_name`、`device_id`、`resolution`、`fps`、`bitrate`、`packet_loss`、`status`、`start_time`、`avg_delay`、`watermark_enabled` | `media_report_stream`同名字段 |
| 流信息列表 | `duration` | 由`media_report_stream.start_time`到`close_time`或当前时间动态计算；缺少`start_time`时回退到上报端`duration` |
...
| 流信息列表 | `current_active_sessions` | 读取`media_report_session`中`close_time`为空的会话后按`stream_id`聚合当前会话数，避免依赖客户端直接上报实时值 |
| 流协议统计 | `protocol_summary[].protocol_type`、`protocol_summary[].total_sessions` | `media_report_stream.protocol_summary`内嵌`JSONB`字段 |
| 流协议统计 | `protocol_summary[].current_sessions` | 读取`media_report_session`中`close_time`为空的会话后按`stream_id + protocol_type`聚合当前会话数 |
| 会话列表 | `stream_info.stream_id`、`stream_info.stream_name`、`stream_info.device_id` | 优先从`media_report_stream`同名字段取当前查询流信息；`media_report_session`也冗余`stream_id`、`stream_name`、`device_id`用于非空列表快速返回 |
| 会话列表 | `protocol_list` | 读取`media_report_stream.protocol_summary`得到协议全集，再按`protocol_type`合并`media_report_session`会话明细 |
| 会话列表 | `protocol_list[].status` | 当前查询范围内`close_time`为空的`media_report_session`按协议计数大于`0`时返回`active`，否则返回`inactive` |
| 会话列表 | `protocol_list[].session_count` | 当前查询范围内按`close_time`为空的`media_report_session.protocol_type`聚合计数 |
| 会话列表 | `active_session_list[].session_id`、`device_id`、`client_id`、`client_ip`、`client_type`、`tenant_id`、`user_name`、`protocol_type`、`start_time`、`current_fps`、`current_bitrate`、`current_resolution`、`node_id`、`instance_id`、`link_hops` | `media_report_session`同名字段 |
| 会话列表 | `play_duration` | 由`media_report_session.start_time`到`close_time`或当前时间动态计算；缺少`start_time`时回退到上报端`play_duration` |
...
- 流信息列表：按`source_type`将请求`source_id`映射为`node_id`或`instance_id`，再结合`tenant_id`、`device_id`或`status`查询`media_report_stream`；`protocol_summary[].total_sessions`直接读取`protocol_summary`，当前活跃会话数按`media_report_session.close_time IS NULL`聚合覆盖。
- 会话列表：按`stream_id`、`tenant_id`、`device_id`、`protocol_type`、`node_id`或`instance_id`查询`close_time`为空的`media_report_session`，链路详情直接读取`link_hops`，链路总延迟优先按`link_hops[].latency_ms`求和。
