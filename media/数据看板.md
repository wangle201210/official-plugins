...
| 总节点概览 | `status`、`CPU`、内存、磁盘、网络、`live_streams`、`sessions`、`avg_delay` | `media_report_node`、`media_report_node_snapshot` |
...
| 流协议统计 | `protocol_summary` | `media_report_stream.protocol_summary` |
| 会话列表 | `session_id`、`client_id`、`tenant_id`、`protocol_type`、播放质量字段 | `media_report_session` |
| 会话链路 | `link_hops`、`total_link_latency` | `media_report_session.link_hops`、`media_report_session.total_link_latency` |

## 接口字段覆盖校验

| 接口区域 | 响应字段 | 数据来源 |
|----------|----------|----------|
| 总节点概览 | `node_id`、`node_name`、`region`、`status`、`parent_node_id`、`total_nodes`、`alive_nodes`、`cpu_allocated`、`cpu_load`、`memory_allocated`、`memory_used`、`disk_io_read`、`disk_io_write`、`network_in`、`network_out`、`live_streams`、`sessions`、`avg_delay`、`last_heartbeat`、`report_time`、`node_latency_map` | `media_report_node`同名字段 |
...
| 流信息列表 | `source_url`、`stream_id`、`stream_name`、`resolution`、`fps`、`bitrate`、`packet_loss`、`status`、`start_time`、`duration`、`avg_delay`、`protocol_count`、`total_sessions_lifetime`、`current_active_sessions`、`watermark_enabled`、`protocol_summary` | `media_report_stream`同名字段 |
| 流协议统计 | `protocol_summary[].protocol_type`、`protocol_summary[].total_sessions`、`protocol_summary[].current_sessions` | `media_report_stream.protocol_summary`内嵌`JSONB`字段 |
...
| 会话列表 | `protocol_list[].status` | `protocol_summary[].current_sessions > 0`时返回`active`，否则返回`inactive` |
| 会话列表 | `protocol_list[].session_count` | 取`protocol_summary[].current_sessions`，或按`media_report_session.protocol_type`计数校验 |
| 会话列表 | `active_session_list[].session_id`、`client_id`、`client_ip`、`client_type`、`tenant_id`、`user_name`、`protocol_type`、`start_time`、`play_duration`、`current_fps`、`current_bitrate`、`current_resolution`、`node_id`、`instance_id`、`link_hops`、`total_link_latency` | `media_report_session`同名字段 |
