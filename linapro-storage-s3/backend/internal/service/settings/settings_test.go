...
func TestSaveMasksSecretAndKeepsOnEmpty(t *testing.T) {
	...
	if _, err = svc.Save(ctx, SaveInput{
		AccessKeyID: "ak-1", Bucket: "demo", Endpoint: "http://minio:9000", ForcePathStyle: true,
	}); err != nil {
...
func TestValidateReadyRequiresProtocolFields(t *testing.T) {
	t.Parallel()
	...
	if err := svc.ValidateReady(&Snapshot{
		AccessKeyID: "a", SecretAccessKey: "s", Bucket: "b", Endpoint: "http://minio:9000",
	}); err != nil {
		t.Fatalf("expected ready without region, got %v", err)
	}
}

func TestEffectiveRegionDefaults(t *testing.T) {
	t.Parallel()
	if got := EffectiveRegion(""); got != DefaultSigningRegion {
		t.Fatalf("got %q want %q", got, DefaultSigningRegion)
	}
	if got := EffectiveRegion(" eu-west-1 "); got != "eu-west-1" {
		t.Fatalf("got %q", got)
	}
}
