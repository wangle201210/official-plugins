...
export const pluginPageMeta = {
  routePath: "linapro-demo-source-sidebar-entry",
...
const pluginAccessCodes = {
  create: "linapro-demo-source:example:create",
  update: "linapro-demo-source:example:update",
  delete: "linapro-demo-source:example:delete",
} as const;
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    columns: [
      {
        field: "title",
        minWidth: 180,
        title: $t("plugin.linapro-demo-source.page.fields.title"),
      },
      {
        field: "content",
        minWidth: 260,
        showOverflow: "ellipsis",
        title: $t("plugin.linapro-demo-source.page.fields.content"),
      },
      {
        field: "attachmentName",
        minWidth: 180,
        slots: { default: "attachment" },
        title: $t("plugin.linapro-demo-source.page.fields.attachment"),
      },
      {
        field: "updatedAt",
        formatter: ({ cellValue }) => formatTimestamp(cellValue),
        title: $t("plugin.linapro-demo-source.page.fields.updatedAt"),
        width: 180,
      },
      {
        field: "action",
    ...
    id: "linapro-demo-source-record-grid",
  },
});
...
async function handleDownloadAttachment(row: DemoRecordItem) {
  ...
  downloadBlob(
    data,
    row.attachmentName || $t("plugin.linapro-demo-source.page.attachmentFallback"),
  );
}
...
<template>
  <Page :auto-content-height="true">
    <Grid :table-title="$t('plugin.linapro-demo-source.page.tableTitle')">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-if="canCreateRecord()"
            data-testid="linapro-demo-source-record-add"
            type="primary"
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            ...
            :data-testid="`linapro-demo-source-record-edit-${row.id}`"
          ...
          <Popconfirm
            ...
            :title="$t('plugin.linapro-demo-source.page.messages.deleteConfirm')"
            @confirm="handleDeleteRecord(row)"
          >
            <ghost-button
              danger
              :data-testid="`linapro-demo-source-record-delete-${row.id}`"
              @click.stop=""
            >
